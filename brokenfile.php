...
if (!empty($HTTP_POST_VARS['submit'])) {
	...
	if ( $sender != 0 ) {
		...
		$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("rha7downloads_broken")." WHERE lid=$lid AND sender=$sender");
		...
		$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("rha7downloads_broken")." WHERE lid=$lid AND ip = '$ip'");
		list ($count)=$xoopsDB->fetchRow($result);
		if ( $count > 0 ) {
			redirect_header("index.php",2,_MD_ALREADYREPORTED);
			exit();
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("rha7downloads_broken")."_reportid_seq");
    $sql = sprintf("INSERT INTO %s (reportid, lid, sender, ip) VALUES (%u, %u, %u, '%s')", $xoopsDB->prefix("rha7downloads_broken"), $newid, $lid, $sender, $ip);
	...
	$xoopsOption['template_main'] = 'rha7downloads_brokenfile.html';
