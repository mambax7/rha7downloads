...
$xoopsOption['template_main'] = 'rha7downloads_viewcat.html';
...
if ( count($arr) > 0 ) {
	...
	foreach($arr as $ele){
		...
		foreach($sub_arr as $sub_ele){
			...
			$infercategories .= "<a href=\"".XOOPS_URL."/modules/rha7downloads/viewcat.php?cid=".$sub_ele['cid']."\">".$chtitle."</a>";
			$space++;
			$chcount++;
		}
...
$fullcountresult=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("rha7downloads_downloads")." WHERE cid=$cid AND status>0");
...
if($numrows>0){
	...
	$q = "SELECT d.lid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description, d.price FROM ".$xoopsDB->prefix("rha7downloads_downloads")." d, ".$xoopsDB->prefix("rha7downloads_text")." t WHERE cid=".$cid." AND d.status>0 AND d.lid=t.lid ORDER BY ".$orderby."";
...
//if 2 or more items in result, show the sort menu
	...
	while(list($lid, $dtitle, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description, $price)=$xoopsDB->fetchRow($result)) {
